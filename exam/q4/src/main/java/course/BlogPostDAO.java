...
public class BlogPostDAO {
    ...
    public List<Document> findByDateDescending(int limit) {
    	List<Document> a = postsCollection.find().sort(descending("date"))
                .limit(limit)
    ...
    public void likePost(final String permalink, final int ordinal) {
    	...
    	if(post == null){
    		System.err.println("not found permalink :" + permalink);
    		return;
    	}
    	ArrayList<Document> co = (ArrayList<Document>) post.get("comments");
    	
    	System.err.println("c:" + co.getClass().getName());
    	Document comment = (Document) ((ArrayList)co).get(ordinal);
    	//num_likes
    	int c = (Integer)comment.get("num_likes") + 1;
    	comment.put("num_likes", c);
    	//postsCollection.replaceOne(eq("permalink", permalink),  post);//new Document("comments" , co)
    	
    	postsCollection.replaceOne(eq("permalink", permalink),  post);//new Document("comments" , co)
    	//postsCollection.replaceOne(eq("permalink", permalink),  new Document("comments." + ordinal , comment));//new Document("comments" , co)
